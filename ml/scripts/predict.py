...
def extract_features(txn: dict, metadata: dict) -> np.ndarray:
...
def score_transaction(txn: dict) -> dict:
    ...
    X = extract_features(txn, metadata)
    ...
    feature_values = dict(zip(features, X[0].tolist()))
